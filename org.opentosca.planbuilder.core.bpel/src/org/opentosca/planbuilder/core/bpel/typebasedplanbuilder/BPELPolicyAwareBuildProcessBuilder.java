...
public class BPELPolicyAwareBuildProcessBuilder extends AbstractBuildPlanBuilder {
     ...
     * </p>
     */
    public BPELPolicyAwareBuildProcessBuilder() {
        try {
            ...
            BPELPolicyAwareBuildProcessBuilder.LOG.error("Error while initializing BuildPlanHandler", e);
        }
        // TODO seems ugly
    ...
    public BPELPlan buildPlan(final String csarName, final AbstractDefinitions definitions,
        ...
        BPELPolicyAwareBuildProcessBuilder.LOG.debug("Created BuildPlan:");
        BPELPolicyAwareBuildProcessBuilder.LOG.debug(getStringFromDoc(newBuildPlan.getBpelDocument()));
        return newBuildPlan;
    ...
    public List<AbstractPlan> buildPlans(final String csarName, final AbstractDefinitions definitions) {
        ...
        for (final AbstractServiceTemplate serviceTemplate : definitions.getServiceTemplates()) {
            ...
            if (!serviceTemplate.hasBuildPlan()) {
                BPELPolicyAwareBuildProcessBuilder.LOG.debug("ServiceTemplate {} has no BuildPlan, generating BuildPlan",
                ...
                BPELPolicyAwareBuildProcessBuilder.LOG.debug("ServiceTemplate {} has BuildPlan, no generation needed",
        ...
        for (final BPELScope templatePlan : buildPlan.getTemplateBuildPlans()) {
            ...
            if (templatePlan.getNodeTemplate() != null) {
                ...
                if (nodeTemplate.getPolicies().isEmpty()) {
                    ...
                    if (plugin != null) {
                        BPELPolicyAwareBuildProcessBuilder.LOG.info("Handling NodeTemplate {} with type plugin {}",
                                                                    nodeTemplate.getId(), plugin.getID());
                        handled = plugin.handleCreate(context, nodeTemplate);
                    } else {
                        BPELPolicyAwareBuildProcessBuilder.LOG.warn("Can't handle NodeTemplate {} with type plugin",
                                                                    nodeTemplate.getId());
                    }
                    ...
                    if (policyPlugin == null) {
                        BPELPolicyAwareBuildProcessBuilder.LOG.debug("Handling NodeTemplate {} with ProvisioningChain",
                                                                     nodeTemplate.getId());
                        final OperationChain chain = BPELScopeBuilder.createOperationChain(nodeTemplate, this.opNames);
                        if (chain == null) {
                            BPELPolicyAwareBuildProcessBuilder.LOG.warn("Couldn't create ProvisioningChain for NodeTemplate {}",
                                                                        nodeTemplate.getId());
                        } else {
                            BPELPolicyAwareBuildProcessBuilder.LOG.debug("Created ProvisioningChain for NodeTemplate {}",
                                                                         nodeTemplate.getId());
                        ...
                        BPELPolicyAwareBuildProcessBuilder.LOG.info("Handling NodeTemplate {} with generic policy aware plugin",
                                                                    nodeTemplate.getId());
                ...
                if (canGenericPluginHandle(relationshipTemplate)) {
                    ...
                    BPELPolicyAwareBuildProcessBuilder.LOG.debug("Couldn't handle RelationshipTemplate {} with type plugin",
                                                                 relationshipTemplate.getId());
                }
    ...
    private boolean canGenericPluginHandle(final AbstractRelationshipTemplate relationshipTemplate) {
        for (final IPlanBuilderTypePlugin plugin : this.pluginRegistry.getTypePlugins()) {
            if (plugin.canHandleCreate(relationshipTemplate)) {
                BPELPolicyAwareBuildProcessBuilder.LOG.info("Found GenericPlugin {} thath can handle RelationshipTemplate {}",
    ...
    public String getStringFromDoc(final org.w3c.dom.Document doc) {
        try {
            ...
            BPELPolicyAwareBuildProcessBuilder.LOG.error("Couldn't transform DOM Document to a String", ex);
            return null;
        }
    }
